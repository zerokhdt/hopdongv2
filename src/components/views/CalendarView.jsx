...
export default function CalendarView({ mode, tasks = [], onTaskClick, colorConfig }) {
  ...
  const renderSummaryView = () => {
    ...
    return (
       <div className="table-container flex flex-col h-full bg-white">
         <div className="grid grid-cols-7 border-b border-[#37352f]/10 bg-[#f7f6f3] flex-shrink-0">
            {weekDays.map(day => (
               <div key={day} className="p-4 text-center text-sm font-bold text-[#37352f]/60 border-r border-[#37352f]/5 last:border-0">{day}</div>
            ))}
         </div>
         <div className="grid grid-cols-7 bg-gray-200 gap-[1px] flex-1">
            {days.map(day => {
                ...
                return (
                  <div key={day.toString()} className={`min-h-[120px] bg-white p-2 flex flex-col ${!isCurrentMonth ? 'bg-gray-50 opacity-60' : ''}`}>
                    <div className={`text-sm font-bold w-8 h-8 flex items-center justify-center rounded-lg mb-2 ${isToday ? 'bg-[#37352f] text-white shadow-lg' : 'text-[#37352f]/40'}`}>
                       {format(day, 'd')}
                    </div>
                      {dayTasks.map(task => {
                         return (
                            <div 
                              ...
                              className="text-sm leading-tight truncate px-2.5 py-2 rounded-lg cursor-pointer text-white font-bold transition-opacity shadow-sm border border-black/5 bg-slate-800"
                              title={task.title}
                            >
                              {task.title}
                            </div>
  ...
  const renderGroupView = () => {
    ...
    return (
      <div className="table-container flex flex-col h-full relative bg-white">
        <div className="overflow-x-auto flex-1 h-full custom-scrollbar flex flex-col bg-slate-200 gap-[1px]">
          ...
          <div style={gridStyle} className="bg-white flex-shrink-0 sticky top-0 z-20 w-max min-w-full border-b border-slate-200">
            ...
            {days.map(day => {
               const isToday = isSameDay(day, new Date());
               return (
                 <div key={day.toString()} className={`p-3 border-r border-slate-200 text-center flex flex-col items-center justify-center ${isToday ? 'bg-[#37352f]/5' : 'bg-slate-50'}`}>
                    <span className={`font-bold text-sm ${isToday ? 'text-[#37352f]' : 'text-slate-600'}`}>
                      {vnDays[getDay(day)]}
                    </span>
                    <span className={`mt-0.5 text-sm ${isToday ? 'text-[#37352f]/50 font-bold' : 'text-slate-400 font-bold'}`}>
                      {format(day, 'dd/MM')}
                    </span>
                 </div>
               )
                       ...
                       {dayTasks.map(task => {
                          ...
                          return (
                            <div 
                              key={task.id} 
                              onClick={() => onTaskClick && onTaskClick(task)}
                              className={`text-sm p-2 flex flex-col gap-1 rounded-lg cursor-pointer border shadow-sm transition-transform hover:scale-[1.02] hover:shadow-md ${colorClass}`}
                              title={`${task.title} (${isPM ? 'Chiều/Tối' : 'Sáng'})`}
                            >
                              <span className="font-bold opacity-80 text-sm tracking-tight block">
                                {format(parseISO(task.startDate), 'HH:mm')} {isPM ? 'PM' : 'AM'}
                              </span> 
                              <span className="font-bold line-clamp-2 leading-tight">{task.title}</span>
                            </div>
                          )
                       })}
                     </div>
              ...
              {[
                ...
                { id: 'month', label: 'Tháng' }
                 ...
                 <button 
                   ...
                   className={`px-3 py-1.5 text-sm font-bold rounded-md transition-all ${timeView === v.id ? 'bg-white shadow-lg text-[#37352f]' : 'text-[#37352f]/40 hover:text-[#37352f]'}`}
                 >
                   {v.label}
                 </button>
              ))}
