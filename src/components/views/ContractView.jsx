...
import { Search, FileText, CheckCircle2, ChevronDown, AlertCircle, Loader2, X, Printer, Filter, Eye, CreditCard, Calendar, Download, ArrowLeft, Monitor } from 'lucide-react';
...
const CONTRACT_FIELDS = [
  { key: 'ma_nv',         label: 'Mã NV',               placeholder: '0xxx',            required: false },
  { key: 'so_hd',         label: 'Số Hợp Đồng',         placeholder: 'HĐLD-0xxx',      required: true },
...
const WORKFLOW_STAGES = [
  { id: 1, label: 'GĐ 1: Khởi tạo & NV Ký', icon: '👤', color: 'red' },
  { id: 2, label: 'GĐ 2: Lãnh đạo ký & Đóng mộc', icon: '🏛️', color: 'emerald' },
  { id: 3, label: 'GĐ 3: Hoàn tất & Link Profile', icon: '✅', color: 'emerald' },
];

function WorkflowAuditTrail({ logs }) {
  return (
    <div className="mt-6 pt-6 border-t border-[#37352f]/10">
      <h5 className="text-sm font-bold text-[#37352f]/40 uppercase tracking-[0.2em] mb-4">Nhật ký quy trình (Audit Trail)</h5>
      <div className="relative space-y-0 pb-4">
        <div className="absolute left-[11px] top-2 bottom-4 w-0.5 bg-[#37352f]/5 z-0" />
        {(logs || []).slice().reverse().map((log, i) => {
          ...
          if (isUpload) dotColor = 'bg-red-500';
          if (isComplete) { dotColor = 'bg-emerald-600'; textColor = 'text-emerald-900 font-bold'; }

          return (
            <div key={i} className="relative z-10 flex items-start gap-5 pb-5 group">
              <div className={`w-6 h-6 rounded-lg ${dotColor} mt-0.5 shrink-0 shadow-lg shadow-black/5 flex items-center justify-center text-sm text-white font-bold`}>
                {isComplete ? '✓' : (i + 1)}
              </div>
              <div className="flex-1">
                <div className={`text-sm font-bold ${textColor} leading-tight mb-1`}>{log.action}</div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-[#37352f]/40 uppercase tracking-widest">{formatDateVN(log.time)}</span>
                  <span className="text-[#37352f]/10 text-sm">|</span>
                  <span className="text-sm font-bold text-red-500">{new Date(log.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  <span className="text-[#37352f]/10 text-sm">|</span>
                  <span className="text-sm font-bold text-[#37352f]/40 italic">bởi {log.user}</span>
                </div>
              </div>
            </div>
          );
...
function ContractWorkflowTracker({ userRole, branch, setTasks }) {
  ...
  const [filterSoHd, setFilterSoHd] = useState('');
      ...
      reader.onload = async () => {
        const base64 = reader.result.split(',')[1];
          const payload = {
            ...
            branch: item.branch || branch || "Khac",
        ...
        if (result.ok) {
          ...
          // Ghi thêm log vào Firestore thông qua API
          apiFetch('/api/contracts/issue-log', {
            method: 'POST',
            body: JSON.stringify({
              issueKey: item.id,
          ...
          alert("Tải lên thành công!");
            ...
            alert(result.error || result.message || "Tải lên thất bại!");
        }
  ...
  const filteredItems = items.filter(it => {
    // Specific filters
    const matchesSoHd = !filterSoHd || (it.soHd || it.so_hd || '').toLowerCase().includes(filterSoHd.toLowerCase());
    const matchesBranchVal = !filterBranch || (it.branch || '').toLowerCase().includes(filterBranch.toLowerCase());
    const matchesPositionVal = !filterPosition || (it.chuc_vu || it.vi_tri || '').toLowerCase().includes(filterPosition.toLowerCase());

    const matchesBranch = (userRole === 'admin') || (it.branch === branch);
    return matchesBranch && matchesSoHd && matchesBranchVal && matchesPositionVal;
  });
    ...
    <div className="relative space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-800">Danh sách quy trình hồ sơ</h3>
          </div>
        </div>
        ...
        <div className="flex flex-1 items-center gap-3">
          <div className="relative group w-40">
            <input 
              type="text"
              placeholder="Số HĐ..."
              className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 outline-none focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 transition-all text-xs font-bold"
              value={filterSoHd}
              onChange={e => setFilterSoHd(e.target.value)}
            />
          </div>
      ...
      <div className="overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-slate-50/50 text-sm font-bold text-slate-500 border-b border-slate-200">
              <th className="px-6 py-5">Nhân sự</th>
              <th className="px-6 py-5">Số hợp đồng</th>
              <th className="px-6 py-5">Chi nhánh</th>
              <th className="px-6 py-5">Vị trí</th>
              <th className="px-6 py-5">Tiến độ quy trình</th>
          ...
          <tbody className="divide-y divide-slate-100">
              {filteredItems.map(it => (
                <tr key={it.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-all group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-indigo-600 font-bold text-xs shrink-0 group-hover:bg-white transition-colors border border-transparent group-hover:border-slate-200">
                        {(it.employeeName || it.ho_ten || 'A').split(' ').slice(-2).map(n => n[0]).join('').toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <div className="text-[14px] font-bold text-slate-800 truncate leading-tight">{formatName(it.employeeName || it.ho_ten)}</div>
                        <div className="text-[11px] font-medium text-slate-400 mt-1 flex items-center gap-1">
                          <CreditCard size={10} />
                          {it.employeeId || it.ma_nv}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 whitespace-nowrap">
                      <FileText size={12} className="text-slate-400" />
                      {it.soHd || it.so_hd || '---'}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-[13px] font-bold text-slate-700">{formatBranch(it.branch)}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-[11px] font-medium text-slate-400">{formatPosition(it.chuc_vu || it.vi_tri || 'N/A')}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1.5">
                      <div className="flex items-center gap-2">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold border ${
                          it.workflowStage === 1 ? 'bg-amber-50 text-amber-700 border-amber-100' :
                          it.workflowStage === 2 ? 'bg-blue-50 text-blue-700 border-blue-100' :
                          it.workflowStage === 3 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                          'bg-slate-50 text-slate-700 border-slate-200'
                        }`}>
                          {WORKFLOW_STAGES[(it.workflowStage || 1) - 1].label}
                        </span>
                        <div className="text-[10px] font-bold text-slate-400 tracking-tighter bg-slate-100 px-1.5 py-0.5 rounded uppercase">
                          {it.workflowStatus === 'COMPLETED' ? 'Hoàn tất' : it.workflowStatus}
                        </div>
                      </div>
                      {(it.submissionDate || it.signingDate) && (
                        <div className="flex items-center gap-2 mt-1 text-[10px] font-bold text-slate-400">
                          <Calendar size={10} className="text-slate-300" />
                          {it.signingDate ? `Ký: ${new Date(it.signingDate).toLocaleDateString('vi-VN')}` : `Trình: ${new Date(it.submissionDate).toLocaleDateString('vi-VN')}`}
                        </div>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-[12px] font-bold text-slate-500 whitespace-nowrap">
                      {it.auditTrail && it.auditTrail.length > 0 ? (
                        <span>{new Date(it.auditTrail.slice(-1)[0].time).toLocaleDateString('vi-VN')}</span>
                      ) : (
                        '--'
                      )}
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="text-[12px] text-slate-400 italic line-clamp-2 max-w-[200px]" title={it.ghi_chu}>
                      {it.ghi_chu || '--'}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right whitespace-nowrap">
                    <button 
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 font-bold text-xs hover:bg-indigo-600 hover:text-white hover:border-indigo-600 hover:shadow-lg hover:shadow-indigo-100 transition-all active:scale-95"
                    >
                      <Eye size={14} />
                      Chi tiết
                    </button>
              ...
              {filteredItems.length === 0 && (
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-gray-500 text-sm font-medium">Chưa có hồ sơ nào trong quy trình.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-5xl max-h-full overflow-hidden rounded-[32px] shadow-2xl border border-white/20 animate-in zoom-in-95 duration-200 flex flex-col">
            ...
            <div className="bg-white border-b border-slate-100 p-6 md:px-8 flex items-center justify-between shrink-0">
              <div>
                <h3 className="text-display-xl text-gray-900 font-bold">{formatName(selectedItem.employeeName || selectedItem.ho_ten)}</h3>
                <p className="text-sm font-bold text-slate-900 tracking-wide mt-1">
                  Số HĐ: <span className="text-base text-black">{selectedItem.soHd || selectedItem.so_hd}</span> · {formatBranch(selectedItem.branch)} · {formatPosition(selectedItem.chuc_vu || selectedItem.vi_tri || 'N/A')}
                </p>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-0">
              ...
              <div className="p-4 md:px-8 md:py-4 bg-slate-50 border-b border-slate-100">
                <div className="flex items-center justify-between mb-4">
                   <div className={`px-4 py-2 rounded-xl font-bold text-[10px] tracking-widest shadow-sm bg-${WORKFLOW_STAGES[(selectedItem.workflowStage || 1) - 1].color}-600 text-white`}>
                ...
                <div className="relative px-6">
                  ...
                  <div className="relative flex justify-between z-10">
                    {WORKFLOW_STAGES.map(s => {
                      ...
                      return (
                        <div key={s.id} className="flex flex-col items-center gap-2 w-32">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg shadow-lg transition-all transform ${isActive ? `bg-${s.color}-600 text-white scale-110` : 'bg-white text-slate-300 border-2 border-slate-200'}`}>
                            {isActive && !isCurrent && s.id < selectedItem.workflowStage ? '✓' : s.icon}
                          </div>
                          <div className={`text-[10px] font-bold text-center tracking-widest leading-tight ${isActive ? 'text-gray-900' : 'text-slate-300'}`}>
                            {s.label}
                          </div>
                        </div>
                      );
                ...
                {/* Scheduling Section */}
                <div className="max-w-3xl mx-auto mb-4 grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-[24px] border border-slate-100">
                  <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-slate-400 tracking-widest ml-1">📅 Ngày trình ký (Trung tâm)</label>
                    <input 
                      type="date"
                      ...
                      className="w-full px-5 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:border-indigo-400 outline-none transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-slate-400 tracking-widest ml-1">✍️ Ngày lãnh đạo ký (Chính thức)</label>
                    <input 
                      type="date"
                ...
                <div className="max-w-3xl mx-auto space-y-4">
                  {(() => {
                    ...
                    if (!req) return null;
                    return (
                      <div className="p-4 bg-slate-900 rounded-[24px] text-white shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-white/10 backdrop-blur-md rounded-xl flex items-center justify-center text-xl shadow-inner">📄</div>
                            <div>
                              <div className="text-[10px] font-bold text-white/40 tracking-[0.2em] mb-0.5">Cấu hình hồ sơ bắt buộc:</div>
                              <div className="text-sm font-bold text-indigo-300 tracking-wide">{req}</div>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {selectedItem.workflowStage === 1 && (
                    <div className="p-5 rounded-[24px] border-2 border-dashed border-red-100 bg-red-50/30">
                      <h4 className="font-bold text-red-900 text-base mb-3 flex items-center gap-2">
                        <span className="w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center text-[10px]">1</span>
                        Nhân viên ký tay & Upload Scan
                      </h4>
                      <p className="text-xs text-red-700/60 font-bold mb-4">Vui lòng in hợp đồng bản cứng, yêu cầu nhân viên ký xác nhận, sau đó scan bản có chữ ký và tải lên tại đây.</p>
                      
                      <div className="flex flex-col md:flex-row gap-4 mt-2 items-stretch">
                        <div className="w-full md:w-[220px] flex flex-col gap-3 shrink-0">
                          <div className="relative group h-full">
                            ...
                            />
                            <label htmlFor="upload-scan-nv" className="flex items-center gap-3 w-full h-full min-h-[88px] px-4 bg-white border-2 border-slate-200 border-dashed rounded-[20px] cursor-pointer group-hover:border-red-400 transition-all shadow-sm">
                              <div className="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center text-xl shrink-0">
                                {uploadLoading ? <Loader2 className="animate-spin" /> : '↑'}
                              </div>
                              <div className="text-left overflow-hidden">
                                <div className="text-[9px] font-black text-red-900 uppercase tracking-widest leading-none mb-1">
                                  {uploadLoading ? 'Đang tải...' : 'Tải hồ sơ quét'}
                                </div>
                                <div className="text-[9px] font-bold text-slate-400 truncate max-w-full">
                                  {selectedItem.nvScanFile || 'File PDF ký tay'}
                                </div>
                              </div>
                            </label>
                          </div>
                          ...
                          {selectedItem.nvScanFile && (
                            <button 
                              ...
                              className="w-full py-3 bg-red-600 text-white font-bold text-[10px] rounded-[14px] shadow-lg shadow-red-100 tracking-[0.2em] hover:bg-red-700 active:scale-[0.98] transition-all"
                            >
                              Xác nhận & GĐ 2 →
                            </button>
                          )}
                        ...
                        <div className="flex-1 bg-white rounded-[20px] p-4 border border-slate-100 flex flex-col shadow-sm h-full">
                          <label className="block text-[9px] font-bold text-black uppercase tracking-widest mb-2 flex items-center gap-1.5 leading-none">
                            <span className="w-1 h-3 bg-red-400 rounded-full"></span> Ghi chú & Nhật ký công việc
                          </label>
                          <textarea 
                            ...
                            className="flex-1 w-full bg-transparent border-none p-0 text-xs font-semibold text-slate-700 outline-none resize-none min-h-[60px]"
                            placeholder="Ghi chú quan trọng cho giai đoạn này..."
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  {selectedItem.workflowStage === 2 && (
                    <div className="p-5 rounded-[24px] border-2 border-dashed border-emerald-100 bg-emerald-50/30">
                      ...
                      <p className="text-xs text-emerald-700/60 font-bold mb-4">Gửi bản cứng cho lãnh đạo ký và đóng mộc. Sau đó scan bản hoàn thiện cuối cùng và cập nhật lên hệ thống.</p>
                      ...
                      <div className="flex flex-col md:flex-row gap-4 mt-2 items-stretch">
                        <div className="w-full md:w-[220px] flex flex-col gap-3 shrink-0">
                          <div className="relative group h-full">
                            ...
                            />
                            <label htmlFor="upload-scan-final" className="flex items-center gap-3 w-full h-full min-h-[88px] px-4 bg-white border-2 border-slate-200 border-dashed rounded-[20px] cursor-pointer group-hover:border-emerald-400 transition-all shadow-sm">
                              <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center text-xl shrink-0">
                                {uploadLoading ? <Loader2 className="animate-spin" /> : '↑'}
                              </div>
                              <div className="text-left overflow-hidden">
                                <div className="text-[9px] font-black text-emerald-900 uppercase tracking-widest leading-none mb-1">
                                  {uploadLoading ? 'Đang cập nhật...' : 'Tải bản lưu trữ'}
                                </div>
                                <div className="text-[9px] font-bold text-slate-400 truncate max-w-full">
                                  {selectedItem.finalScanFile || 'Hồ sơ mộc & ký GĐ'}
                                </div>
                              </div>
                            </label>
                          </div>
                          ...
                          {selectedItem.finalScanFile && (
                            <button 
                              ...
                              className="w-full py-3 bg-emerald-600 text-white font-bold text-[10px] rounded-[14px] shadow-lg shadow-emerald-100 tracking-[0.2em] hover:bg-emerald-700 active:scale-[0.98] transition-all"
                            >
                              Xác nhận hoàn tất ✓
                            </button>
                          )}
                        ...
                        <div className="flex-1 bg-white rounded-[20px] p-4 border border-slate-100 flex flex-col shadow-sm h-full">
                          <label className="block text-[9px] font-bold text-black uppercase tracking-widest mb-2 flex items-center gap-1.5 leading-none">
                            <span className="w-1 h-3 bg-emerald-400 rounded-full"></span> Ghi chú & Nhật ký công việc
                          </label>
                          <textarea 
                            value={selectedItem.ghi_chu || ''}
                            onChange={(e) => {
                  ...
                  {selectedItem.workflowStage === 3 && (
                    <div className="p-8 rounded-[32px] bg-emerald-600 text-white shadow-2xl relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-6 opacity-10 text-7xl grayscale transform rotate-12">📝</div>
                      <div className="relative z-10">
                        <div className="flex items-center gap-4 mb-6">
                          <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center text-3xl shadow-inner">🎉</div>
                          <div>
                            <h4 className="font-bold text-xl tracking-wider mb-1">Quy trình hoàn tất</h4>
                            <p className="text-[10px] font-bold text-white/70 tracking-[0.2em]">Hồ sơ đã được số hóa & lưu trữ an toàn</p>
                          </div>
                        </div>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="p-4 bg-white/10 backdrop-blur-sm rounded-2xl border border-white/10 group cursor-pointer hover:bg-white/20 transition-all">
                            <div className="text-[9px] font-bold text-white/50 tracking-widest mb-1">Quét nhân viên</div>
                            <div className="text-xs font-bold truncate">{selectedItem.nvScanFile}</div>
                          </div>
                          <div className="p-4 bg-white/10 backdrop-blur-sm rounded-2xl border border-white/10 group cursor-pointer hover:bg-white/20 transition-all">
                            <div className="text-[9px] font-bold text-white/50 tracking-widest mb-1">Quét hoàn tất</div>
                            <div className="text-xs font-bold truncate">{selectedItem.finalScanFile}</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Note Section (Visible in Stage 3 or other states) */}
                  {selectedItem.workflowStage === 3 && (
                    <div className="bg-slate-50 rounded-[24px] p-5 border border-slate-100">
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-3">Ghi chú & Nhật ký công việc</label>
                      <textarea 
                        value={selectedItem.ghi_chu || ''}
                        onChange={(e) => {
                          const v = e.target.value;
                        ...
                        className="w-full p-4 rounded-[16px] border border-slate-200 bg-white text-sm font-semibold outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all min-h-[80px] resize-none"
...
function validateContractData(data, { contractTypeId, agreementTypeId }) {
  ...
  const _needDen = requiresDenNgay(contractTypeId, agreementTypeId);
...
function _saveContractTypeDocs(map) {
...
function _saveAgreementDocs(map) {
  const safe = map && typeof map === 'object' && !Array.isArray(map) ? map : {};
...
function _saveCommitmentDocs(map) {
  const safe = map && typeof map === 'object' && !Array.isArray(map) ? map : {};
...
function computeSoHdFromMaNv(maNv) {
  ...
  if (!id) return '';
  // logic map: id (4 số) -> số hđ? (HĐLD-xxxx)
  return `HĐLD-${id}`;
}
...
const PAGE_CSS = `
  ...
  .page p { margin: 0 0 0; line-height: 1.5; min-height: 1.5em; }
  ...
  .section-title { font-weight: 700; margin-top: 10px; margin-bottom: 2px; }
  ...
  .country-title { font-size: 16pt; font-weight: 700; line-height: 1.3; }
  .main-title { margin: 16px 0 16px; font-size: 20pt; font-weight: 700; text-align: center; letter-spacing: 0.02em; }
  ...
  /* docx-preview override - Enforce A4 standard */
  .docx-wrapper {
  ...
  /* Responsive scaling for A4 Review on Tablets/Small Laptops */
  .a4-responsive-scaler {
    display: flex;
  ...
  @media (max-width: 850px) {
    .a4-responsive-scaler {
      ...
      height: calc(var(--page-height) * 0.75);
    }
  }
  ...
  @media (max-width: 640px) {
    .a4-responsive-scaler {
      transform: scale(0.6);
      height: calc(var(--page-height) * 0.6);
    }
  }
...
// (HdldFtTemplate và HdldBaseTemplate đã được chuyển sang ./contracts/)
...
// ─── Component: Live docx-preview renderer ───────────────────────────────────
function DocxLivePreview({ templateUrl, data }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await fetch(templateUrl, { cache: 'no-store' });
        if (!resp.ok) throw new Error(`Không tải được template (${resp.status}). Hãy kiểm tra file tồn tại ở /public/templates/`);
        const buf = await resp.arrayBuffer();
        const zip = new PizZip(buf);
        const delimiters = (() => {
          try {
        ...
        await renderAsync(outputBlob, containerRef.current, null, {
        ...
        if (!cancelled) {
          setError(e.message || 'Lỗi render docx preview');
          setLoading(false);
        }
      }
    })();
    ...
    <div className="relative">
      {loading && (
        <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-500">
          <Loader2 size={28} className="animate-spin text-[#00288e]" />
          <span className="text-sm font-medium">Đang tải bản xem trước Word...</span>
          <span className="text-xs text-slate-400">Đang điền dữ liệu vào template .docx</span>
        </div>
      )}
      {error && (
        <div className="mx-auto w-[210mm] mt-6 p-5 bg-red-50 border border-red-200 rounded-2xl text-red-600 text-sm font-medium leading-relaxed">
          <div className="font-bold mb-1">⚠️ Không thể hiển thị bản xem trước Word</div>
          <div className="text-xs text-red-500">{error}</div>
          <div className="mt-3 text-xs text-red-400 italic">Gợi ý: Bản HTML bên trái vẫn có thể dùng để tải .docx bình thường.</div>
        </div>
      )}
      <div className="a4-responsive-scaler">
        <div
          ref={containerRef}
          className={`transition-opacity duration-300 ${loading ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}
        />
      </div>
    </div>
  );
...
export function ContractPreview({ data, contractTypeId, position: _position, agreementTypeId, commitmentTypeId: _commitmentTypeId, onEdit, onPrint: _onPrint, isPrinting }) {
  ...
  const [docxTemplateFile, _setDocxTemplateFile] = useState(null);
  ...
  const _previewEmployees = React.useMemo(() => {
  ...
  const _contractVariant = contractGroup?.variants.find(v => v.id === vId) || contractGroup?.variants.find(v => v.id === 'hd-fulltime') || null;
  ...
  const _ky   = fmt(localData.ngay_ky);
  const _sinh = fmt(localData.ngay_sinh);
  const _cap  = fmt(localData.ngay_cap);
  const _tu   = fmt(localData.tu_ngay);

  const V = ({ v, blank = '...............' }) => <span className="red">{v || blank}</span>;

  let _pageNum = 1;

  const _handleSelectPreviewEmployee = (s) => {
    const next = {
      ...localData,
      ma_nv: s.id,
  ...
  const _isTrialBorder = String(soHdBase(localData.so_hd || '')).trim() && String(soHdBase(localData.so_hd || '')).trim() === String(localData.ma_nv || '').trim();

  // Tính URL template cho docx-preview
  const docxPreviewUrl = React.useMemo(() => {
    const labels = [
      agreementTypeId ? optionLabel(AGREEMENT_DOC_TYPE_OPTIONS, agreementTypeId) : null,
      contractTypeId ? optionLabel(CONTRACT_DOC_TYPE_OPTIONS, contractTypeId) : null,
    ].filter(Boolean);
    for (const label of labels) {
      const url = getTotalHdDocxUrlFromLabel(label);
      if (url) return url;
    }
    // fallback về template mặc định
    if (contractTypeId === 'hdld-ft' || !contractTypeId) return '/templates/hdld-ft.docx';
    return '';
  }, [contractTypeId, agreementTypeId]);

  return (
    <div className="min-h-screen flex flex-col items-center pv-wrapper py-10"
      style={{ background: 'linear-gradient(160deg, #8da3b8 0%, #b0bec8 40%, #9aacbb 100%)' }}
      id="print-root"
    >
      <style dangerouslySetInnerHTML={{ __html: PAGE_CSS }} />

      {/* ── Toolbar ── */}
      <div className="no-print w-[210mm] mb-6 sticky top-4 z-10">
        <div className="flex gap-2 p-2 bg-white/75 backdrop-blur-md rounded-2xl shadow-xl border border-white/60">
          {/* Quay lại */}
          <button
            onClick={onEdit}
            className="flex items-center gap-2 px-4 py-2.5 bg-slate-100/80 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-all text-sm shrink-0"
          >
            <ArrowLeft size={15} /> Chỉnh sửa
          </button>

          {/* Badge trạng thái */}
          <div className="flex-1 flex items-center justify-center gap-2 px-3 rounded-xl bg-slate-100/60">
            <Monitor size={13} className="text-[#00288e]" />
            <span className="text-xs font-bold text-[#00288e] tracking-wide">Xem trước nội dung HĐLĐ</span>
          </div>

          {/* Tải docx */}
          <button
            onClick={handleDownloadWord}
            disabled={docxBusy || isPrinting}
            className="flex items-center gap-2 px-4 py-2.5 bg-[#00288e] hover:bg-blue-900 text-white font-extrabold rounded-xl transition-all text-sm shadow-lg shadow-blue-900/30 disabled:opacity-50 shrink-0"
          >
            {docxBusy
              ? <><Loader2 size={14} className="animate-spin" /> Đang tạo...</>
              : <><Download size={14} /> TẢI HĐLĐ</>}
          </button>
        </div>
      </div>

      {docxError && (
        <div className="no-print w-[210mm] -mt-2 mb-6 bg-white/90 border border-red-200 rounded-xl p-4 shadow-sm">
      ...
      {/* ── Word Review (luôn hiển) ── */}
      <div className="w-full">
        <DocxLivePreview templateUrl={docxPreviewUrl} data={localData} />
      </div>
    </div>
  );
}
...
// ─── ContractView — Main ──────────────────────────────────────────────────────
export default function ContractView({ onLogout: _onLogout, employees = [], userRole, initialEmployeeId, initialMode, branch, setTasks }) {
  ...
  const [reviewContractDocx, _setReviewContractDocx] = useState(null);
  const [reviewAgreementDocx, _setReviewAgreementDocx] = useState(null);
  const [reviewCommitmentDocx, _setReviewCommitmentDocx] = useState(null);
  const _contractDocxRef = useRef(null);
  const _agreementDocxRef = useRef(null);
  const _commitmentDocxRef = useRef(null);
  ...
  const [_contractTypeDocs, _setContractTypeDocs] = useState(() => loadContractTypeDocs());
  const [_agreementDocs, _setAgreementDocs] = useState(() => loadAgreementDocs());
  const [_commitmentDocs, _setCommitmentDocs] = useState(() => loadCommitmentDocs());
  const [mode, setMode] = useState(initialMode || 'search');
  const [quickRows, setQuickRows] = useState([]);
  const [_quickError, setQuickError] = useState('');
  const [quickQuery, _setQuickQuery] = useState('');
  const [pasteText, setPasteText] = useState('');
  const [_quickLoading, setQuickLoading] = useState(false);
  const [_quickFileName, setQuickFileName] = useState('');
  ...
  const [quickPrintInfoCollapsed, _setQuickPrintInfoCollapsed] = useState(() => localStorage.getItem('ace_quick_print_info_collapsed') === '1');
  ...
  const _filteredQuickRows = React.useMemo(() => {
  ...
  const _requiredDocs = React.useMemo(() => {
    const mappings = JSON.parse(localStorage.getItem('ace_position_contract_mapping_v1') || '{}');
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-100 font-sans">
      <div className="w-full max-w-[1600px] mx-auto py-8 px-4 md:px-8 shrink-0">
        {mode === 'workflow' ? (
          /* ── Giao diện QUY TRÌNH HỒ SƠ ── */
          <div className="space-y-8">
            <div className="mb-6">
              <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Hợp đồng & Hồ sơ</h1>
            </div>
            <div className="bg-white rounded-[32px] border border-slate-200 p-6 shadow-xl shadow-slate-200/50 min-h-[600px] transition-all">
          ...
          <div className="space-y-8">
            <div className="mb-6">
              <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">Tạo Hợp đồng & Phụ lục</h1>
            </div>
            {step === 'input' && (
