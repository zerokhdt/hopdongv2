...
function OnboardingForm({ onSubmit, maskSalary: _maskSalary = false, uploadLoading = false }) {
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-6 mb-10">
        <div className="flex-1">
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              <h3 className="text-2xl font-bold text-[#37352f] tracking-tight leading-none mb-2">Báo tăng Nhân sự</h3>
              <p className="text-sm font-bold text-[#37352f]/40 uppercase tracking-[0.2em]">Quy trình tự động hóa hồ sơ chi nhánh</p>
            </div>
          </div>
        </div>
        <div className="w-full md:w-80">
           <label className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-2 block ml-1">Lấy từ HĐ đã in (Hỗ trợ nhập liệu)</label>
           <select 
          ...
          <div className="bg-white rounded-[40px] border border-slate-100 shadow-2xl shadow-slate-200/40 p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="md:col-span-2 flex items-center gap-3">
                ...
                <span className="text-xs font-bold text-gray-900 uppercase tracking-[0.2em]">HỒ SƠ NHÂN SỰ</span>
              </div>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <label htmlFor="onboarding-scan" className="flex flex-col items-center justify-center gap-4 w-full py-12 bg-slate-50 border-2 border-slate-100 border-dashed rounded-[32px] cursor-pointer group-hover:border-indigo-400 group-hover:bg-indigo-50/30 transition-all">
              ...
              <div className="text-center">
                <span className="text-xs font-black text-slate-600 uppercase tracking-widest block mb-1">
                  {formData.scanFile ? 'Đã tải hồ sơ quét' : 'Chọn bản Scan PDF ký tay'}
                </span>
          ...
          {requiredDocs && (
            <div className="bg-white rounded-[40px] border border-slate-100 p-8 shadow-xl shadow-slate-200/30">
               <h4 className="text-xs font-bold text-gray-900 uppercase tracking-[0.2em] mb-6 flex items-center gap-3">
                 <span className="w-8 h-8 border-2 border-slate-800 text-gray-900 rounded-xl flex items-center justify-center text-xs">✓</span>
        ...
        <div className="col-span-2">
          ...
          <div className="col-span-2 p-3 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-between text-xs">
            <div className="flex gap-4">
              <div><span className="text-slate-400">Vị trí hiện tại:</span> <span className="font-bold">{formatPosition(selectedEmp.position)}</span></div>
              <div><span className="text-slate-400">Phòng ban:</span> <span className="font-bold">{formatBranch(selectedEmp.department)}</span></div>
            </div>
          </div>
