...
export default function SettingsModal({ currentConfig, onSave, onClose, groups, onUseDemoData }) {
  const [config, setConfig] = useState(() => JSON.parse(JSON.stringify(currentConfig)));
  const [_mappingVersion, setMappingVersion] = useState(0);
  ...
  const saveAndClose = () => {
    ...
    localStorage.setItem('ace_hrm_font_scale', fontScale.toString());
    document.documentElement.style.setProperty('--base-scale', fontScale);
    onSave(config);
    onClose();
  };
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-8">
          ...
          <section className="bg-slate-50 p-5 rounded-[24px] border border-slate-200">
            <h3 className="text-xs font-black text-indigo-500 tracking-widest mb-4 flex items-center gap-2">
                ...
                {Object.entries(JSON.parse(localStorage.getItem('ace_position_contract_mapping_v1') || '{}')).map(([pos, docs]) => (
                  <div key={pos} className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl">
                    <div>
                      ...
                      <div className="text-[9px] font-bold text-indigo-500 italic">{docs}</div>
                    </div>
                    <button 
                      onClick={() => {
                        const cur = JSON.parse(localStorage.getItem('ace_position_contract_mapping_v1') || '{}');
          ...
          {/* Status Colors */}
          <section>
            <h3 className="text-xs font-bold text-slate-500 tracking-wider mb-3 flex items-center gap-2">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-4">
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  <p className="text-sm font-semibold text-slate-700">Kích thước chữ</p>
                  <span className="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-md border border-orange-100 italic">
                ...
                <input 
                  ...
                  onChange={(e) => {
                    ...
                    setFontScale(val);
                    // Live preview
                    document.documentElement.style.setProperty('--base-scale', val);
                  }}
                  className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-orange-500"
                />
                <div className="flex justify-between text-[10px] font-bold text-slate-400 px-1 uppercase tracking-tight">
                  <span>Nhỏ</span>
                  <span>Mặc định</span>
                  <span>Lớn</span>
                </div>
          ...
          <section className="pt-4 border-t border-slate-100">
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 space-y-4 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-black text-amber-900">Chế độ Demo Khách hàng</h4>
                  <p className="text-xs text-amber-700 mt-1 leading-relaxed">
                    Tự động tạo 300 nhân viên phẩn bổ 10 chi nhánh & 180 hồ sơ tuyển dụng thực tế để trình diễn tính năng.
                  </p>
                </div>
              </div>
              
              <div className="space-y-2">
                <p className="text-[10px] font-bold text-amber-600 uppercase">Nhập mã xác nhận (147134)</p>
                <input 
                  type="password"
                  id="demo-pin-input"
                  placeholder="******"
                  className="w-full px-4 py-2 bg-white border border-amber-200 rounded-xl text-center font-black tracking-[0.5em] outline-none focus:border-amber-500 shadow-inner"
                />
              </div>
              ...
              <button
                type="button"
                onClick={() => {
                  const pin = document.getElementById('demo-pin-input').value;
                  if (pin === '147134') {
                    if (window.confirm('Hệ thống sẽ ghi đè toàn bộ dữ liệu hiện tại bằng dữ liệu demo (300 nhân viên, 180 ứng viên). Bạn có chắc chắn?')) {
                      if (onUseDemoData) {
                        onUseDemoData();
                        onClose();
                      }
                ...
                className="w-full py-3 bg-amber-600 text-white rounded-xl text-xs font-black shadow-md shadow-amber-600/20 hover:bg-amber-700 transition-all flex items-center justify-center gap-2"
              >
              ...
              <p className="text-[10px] text-amber-500 text-center font-bold uppercase tracking-tighter">
                * Lưu ý: Dữ liệu cũ sẽ bị thay thế
              </p>
            </div>
          </section>
        </div>
