...
export default function TaskModal({ task, onClose, onSave, groups = [], teamMembers = [], userRole = 'user' }) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 lg:p-10">
      <div className="bg-white rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-gray-100 w-full max-w-6xl h-full max-h-[95vh] flex flex-col overflow-hidden">
        
        {/* Header & Meta Properties */}
        <div className="px-6 md:px-8 py-5 flex flex-col border-b border-gray-100 bg-white flex-shrink-0 relative">
          <button onClick={onClose} className="absolute top-5 right-6 p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0 z-10">
            <X size={24} />
          </button>
          ...
          <div className="flex-1 pr-12">
            <div className="flex items-center gap-3 mb-4">
              ...
              <input
                type="text"
                disabled={!isAdmin}
                className={`text-2xl font-semibold text-gray-900 bg-transparent border-none outline-none w-full placeholder-gray-300 ${!isAdmin ? 'cursor-not-allowed' : ''}`}
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-4 pl-9">
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Trạng thái</span>
                <select
                  className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium text-sm rounded-md px-3 py-1.5 border-none outline-none cursor-pointer transition-colors"
              ...
              {/* Priority */}
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Ưu tiên</span>
                <select
                  disabled={!isAdmin}
                  className={`bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium text-sm rounded-md px-3 py-1.5 border-none outline-none cursor-pointer transition-colors ${!isAdmin ? 'cursor-not-allowed' : ''}`}
              ...
              {/* Group */}
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Chi nhánh nhận task</span>
                <select
                  disabled={!isAdmin}
                  className={`bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium text-sm rounded-md px-3 py-1.5 border-none outline-none cursor-pointer transition-colors ${!isAdmin ? 'cursor-not-allowed' : ''}`}
